...
export function useScroll(chatAreaRef: React.RefObject<HTMLElement | null>) {
  const autoScroll = useStreamStore(s => s.autoScroll);
  const setAutoScroll = useStreamStore(s => s.setAutoScroll);

  const pinToBottom = useCallback((area: HTMLElement | null): void => {
    if (!area) return;
    area.scrollTop = area.scrollHeight;
  }, []);

  const scrollToBottom = useCallback((force = true): void => {
  ...
  const maybeScroll = useCallback((): void => {
