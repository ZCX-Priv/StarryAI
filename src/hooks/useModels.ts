...
export default function useModels() {
  ...
  const loadModels = useCallback(async (): Promise<ModelInfo[] | null> => {
  ...
  const filterModels = useCallback((searchTerm: string): ModelInfo[] => {
  ...
  const setModel = useCallback(async (id: string): Promise<void> => {
    useModelStore.getState().setModel(id);
    await IDBStore.setConfig('model', id);
  }, []);

  const setModelAndUpdate = useCallback(async (id: string): Promise<void> => {
    useModelStore.getState().setModel(id);
    await IDBStore.setConfig('model', id);
