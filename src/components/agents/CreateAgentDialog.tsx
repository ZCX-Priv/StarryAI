...
export default function CreateAgentDialog({ visible, onClose }: CreateAgentDialogProps) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (agentsConfig && agentsConfig.agents) {
        agentsConfig.agents.push(agent as unknown as AgentItem);
  ...
  return (
    <div className="modal-overlay visible" onClick={(e: React.MouseEvent<HTMLDivElement>) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal" style={{ maxWidth: '520px' } as React.CSSProperties}>
        ...
        <div className="modal-body">
          <div className="form-group">
            ...
            <input type="text" className="form-input" placeholder="例如：代码审查助手" maxLength={20} value={name} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setName(e.target.value)} />
          ...
          <div className="form-group">
            ...
            <div className="emoji-picker-container">
              ...
              <input type="text" className="emoji-input" placeholder="或输入任意emoji" maxLength={2} value={emoji} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEmoji(e.target.value)} />
            </div>
            {showEmojiGrid && (
          ...
          <div className="form-group">
            ...
            <select className="category-select" value={category} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setCategory(e.target.value)}>
          ...
          <div className="form-group">
            ...
            <input type="text" className="form-input" placeholder="简短描述这个智能体的用途" maxLength={100} value={description} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setDescription(e.target.value)} />
            <div className="form-hint">最多100个字符（可选）</div>
          </div>
          <div className="form-group">
            <label className="form-label">角色定义 <span className="required">*</span></label>
            <textarea className="form-textarea" placeholder="定义这个智能体的角色、能力和行为方式..." maxLength={2000} value={prompt} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setPrompt(e.target.value)} />
