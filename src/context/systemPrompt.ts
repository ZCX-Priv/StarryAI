...
import { getCurrentTimeInfo } from './time';
import { buildSystemPromptFromTemplate, parsePromptTemplate } from '@/lib/prompts';
...
export function buildMemoryExtractPrompt(existingMemory: string, conversation: Message[]): string {
  const modeState = useModeStore.getState();
...
export function buildMemoryDeduplicatePrompt(memoryList: string[], maxEntries: number): string {
  const modeState = useModeStore.getState();
