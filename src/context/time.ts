...
export function solar2lunar(y: number, m: number, d: number): LunarResult | null {
...
export function getYearCn(year: number): string {
  const gan = (year - 4) % 10;
  const zhi = (year - 4) % 12;
  return tianGan[gan] + diZhi[zhi];
}

export function getSolarTerm(y: number, m: number, d: number): string | null {
  const baseDate = new Date(1900, 0, 6, 2, 5, 0);

  for (let i = (m - 1) * 2; i < m * 2; i++) {
    const tmp1 = new Date(baseDate.getTime() + solarTermInfo[i] * 60000);
...
export function getFestival(y: number, m: number, d: number, lunarMonth: number, lunarDay: number): string | null {
  const solarKey = `${String(m).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
  const solarFestival = solarFestivals[solarKey];
...
export function getCurrentTimeInfo(): string {
  try {
    ...
    if (!lunar) {
      return `今天的日期：${year}年${String(month).padStart(2, '0')}月${String(day).padStart(2, '0')}日星期${weekDays[weekDay]}`;
    }

    const festival = getFestival(year, month, day, lunar.month, lunar.day);
    const festivalStr = festival ? `，今日节日/节气：${festival}` : '，今日节日/节气：无';

    return `今天的日期：${year}年${String(month).padStart(2, '0')}月${String(day).padStart(2, '0')}日星期${weekDays[weekDay]}，农历：${lunar.yearCn}年${lunar.monthCn}${lunar.dayCn}(${lunar.animal}年)${festivalStr}`;
  } catch (error) {
